...
slug: development__declarative-agent__spe-da-adv
persona: developer
scenario_ids: [D10]
source_of_truth: https://learn.microsoft.com/en-us/sharepoint/dev/embedded/development/declarative-agent/spe-da-adv
source_date: 03/06/2025
last_verified: 2026-06-23
agent_ready: true
---

# SharePoint Embedded agent Advanced Topics

> Learn how the semantic index powers Retrieval-Augmented Generation (RAG) to provide accurate, context-aware AI responses in SharePoint Embedded agent.

**Agent task:** Answer SharePoint Embedded questions about "SharePoint Embedded agent Advanced Topics" using only this page. Use the exact Microsoft Graph calls, permissions, parameters, and steps below, respect the developer persona boundary, and cite this page.
...
The [`discoverabilityDisabled`](https://learn.microsoft.com/en-us/sharepoint/dev/embedded/administration/developer-admin/dev-admin#container-type-configuration-properties) property controls whether Microsoft 365 can discover [drive items](https://learn.microsoft.com/graph/api/resources/driveitem) within a specific container type.
...
Here's an example of how to set `discoverabilityDisabled` to `false` with [Set-SPOContainerTypeConfiguration](https://learn.microsoft.com/powershell/module/SharePoint-online/set-spocontainertypeconfiguration#examples)
...
![Using the VS Code extension for SPE to set DiscoverabilityDisabled to false](https://learn.microsoft.com/en-us/sharepoint/dev/embedded/images/speco-vscodeextensiondisablediscovery.png)
...
- [Set-SPOApplication](https://learn.microsoft.com/powershell/module/SharePoint-online/set-spoapplication) to set the `CopilotEmbeddedChatHosts` property.
- [Get-SPOApplication](https://learn.microsoft.com/powershell/module/SharePoint-online/get-spoapplication) to get the `CopilotEmbeddedChatHosts` property.
...
![Venn Diagram with SPE application access on left, SPE agent in middle and consuming tenant user on right, overlapped area is what agent can access](https://learn.microsoft.com/en-us/sharepoint/dev/embedded/images/speco-appscopingvenn.png)

### Information Architecture

Files in SharePoint Embedded are naturally [semantic indexed](https://learn.microsoft.com/en-us/sharepoint/dev/embedded/development/declarative-agent/spe-da-adv#semantic-index). This semantic index underpins retrieval augmented generation [(RAG)](https://learn.microsoft.com/en-us/sharepoint/dev/embedded/development/declarative-agent/spe-da-adv#retrieval-augmented-generation-rag) workflows by providing relevant context from your stored content at query time. In essence, it [grounds](https://learn.microsoft.com/en-us/sharepoint/dev/embedded/development/declarative-agent/spe-da-adv#grounding) the AI responses, ensuring they directly reference accurate information in your containers rather than relying on general knowledge alone.

![How RAG works in SPE](https://learn.microsoft.com/en-us/sharepoint/dev/embedded/images/speco-ragm365.png)

With SharePoint Embedded agent, you can further ground the large language models (LLM) response on [specific files or drive items.](https://learn.microsoft.com/en-us/sharepoint/dev/embedded/development/declarative-agent/spe-da-adv#scoping-your-agent-to-specific-content).

### Semantic index

[Learn more about semantic index for Microsoft 365 Copilot here](https://learn.microsoft.com/microsoftsearch/semantic-index-for-copilot)
...
​![The flow of a RAG query](https://learn.microsoft.com/en-us/sharepoint/dev/embedded/images/speco-ragquery.png)

### Grounding

Grounding in the context of SPE agent refers to the process of providing input sources to the large language model (LLM) related to the user's prompt. This helps improve the specificity of the prompt and ensures that the responses are relevant and actionable to the user's specific task. The data the agent is grounded on will be the contents of the container type in the agent application. Behind the scenes, SPE agent uses Microsoft 365 Copilot. [Learn more about Microsoft 365 Copilot architecture](https://learn.microsoft.com/copilot/microsoft-365/microsoft-365-copilot-architecture).
...
---
*Agent-first reformat of the official Microsoft Learn doc (content preserved). Source of truth: https://learn.microsoft.com/en-us/sharepoint/dev/embedded/development/declarative-agent/spe-da-adv*
