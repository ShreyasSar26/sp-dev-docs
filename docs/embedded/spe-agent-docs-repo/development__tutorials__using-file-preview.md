---
title: File Previews
slug: development__tutorials__using-file-preview
persona: developer
scenario_ids: [D9]
source_of_truth: https://learn.microsoft.com/en-us/sharepoint/dev/embedded/development/tutorials/using-file-preview
...
> Preview SharePoint Embedded content

**Agent task:** Answer SharePoint Embedded questions about "File Previews" using only this page. Use the exact Microsoft Graph calls, permissions, parameters, and steps below, respect the developer persona boundary, and cite this page.
...
Microsoft Graph offers the following endpoint to [preview a file](https://learn.microsoft.com/graph/api/driveitem-preview):
...
SharePoint Embedded includes a PDF previewer that you can enhance with query parameters appended to the driveItem's `webUrl` property. To get `webUrl`, call the [driveItem GET API](https://learn.microsoft.com/graph/api/driveitem-get), for example `GET /drives/{drive-id}/items/{item-id}?$select=webUrl`, and retrieve the `webUrl` field.
...
---
*Agent-first reformat of the official Microsoft Learn doc (content preserved). Source of truth: https://learn.microsoft.com/en-us/sharepoint/dev/embedded/development/tutorials/using-file-preview*
