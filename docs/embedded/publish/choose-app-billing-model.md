...
| --- | --- | --- | --- |
| Standard billing | Developer tenant | Developer admin | You include SPE consumption in your product price or centralize billing. |
| Pass-through billing | Consuming tenant | Customer admin | The customer pays Microsoft directly for their SPE consumption. |

For current billing details, see [PAYG billing for SharePoint Embedded](../administration/billing/billing.md).
...
For more about container types, see [Create New SharePoint Embedded Container Types](../getting-started/containertypes.md).
...
The developer admin must establish a valid Azure billing profile for the container type.

Choose standard billing when:

- You sell a bundled SaaS price and absorb or allocate SPE usage internally.
...
Standard billing usually reduces customer onboarding steps because the customer doesn't need to set up an SPE billing profile in the consuming tenant.
...
The billing source documentation states that the developer admin supplies the Azure subscription, resource group, and region when setting up standard billing.
Use the linked source article for current PowerShell syntax rather than copying stale examples into customer handoffs.

> [!NOTE]
> Standard billing doesn't remove the need for customer consent, tenant registration, or validation.
> It only changes who configures and pays for SPE consumption.
...
The developer admin creates the container type with pass-through billing enabled.
...
- Customers must pay Microsoft directly for their own SPE usage.
...
Before a customer can use a pass-through SPE app, the consuming tenant needs:
...
No user can access any pass-through SPE app before valid billing is set up for the SharePoint Embedded platform in the consuming tenant.
...
1. Your organization owns the SPE consumption billing relationship.
...
1. The customer doesn't need to configure SPE pay-as-you-go billing for this app.
...
1. The customer admin must turn on billing for SPE apps.
...
| How is your product priced? | SPE consumption is bundled. | SPE consumption is separate. |
...
- Sending cmdlet snippets without linking to the current source documentation.
...
> [Guide customers through tenant setup](customer-tenant-setup-guide.md)
