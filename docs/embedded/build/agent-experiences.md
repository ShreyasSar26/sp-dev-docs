...
> [!CAUTION]
> The earlier **SharePoint Embedded agent SDK** (the React `ChatEmbedded` control) was **deprecated in March 2026** and replaced by [Microsoft Foundry Agent Service](/azure/foundry/agents/overview) with a [SharePoint knowledge source (preview)](/azure/search/agentic-knowledge-source-how-to-sharepoint-remote) configured for SharePoint Embedded. Use the Foundry knowledge source for new work. For existing integrations, see [SharePoint Embedded agent (deprecated)](../development/declarative-agent/spe-da.md) and [Set up SPE as a knowledge source in Microsoft Foundry](../development/declarative-agent/sharepoint-embedded-knowledge-source.md).
