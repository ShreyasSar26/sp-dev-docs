...
If you used the SharePoint Embedded Visual Studio Code extension, export the Postman environment to find `ContainerTypeId` and `ClientID`. You can also retrieve the container type ID with `Get-SPOContainerType`, and you can find the client ID in Microsoft Entra ID under **App registrations**.

## Run the item counter sample
The source article points to the `item-counter-spe` sample in the Fluid Examples repository.
