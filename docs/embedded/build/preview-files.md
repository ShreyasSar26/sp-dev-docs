...
For the full tutorial, see [Using File Previews](../development/tutorials/using-file-preview.md).
...
The source tutorial shows this C# SDK pattern:
```csharp
...
> [!CAUTION]
> The source tutorial notes that `getUrl` currently contains an encrypted token that can only be used with your application. This behavior may change.
## Remove the preview banner
The source tutorial notes that you can add `nb=true` to the obtained URL to remove the banner at the top.
Example:
```http
...
The source tutorial shows this server-side pattern:
```csharp
[HttpGet]
