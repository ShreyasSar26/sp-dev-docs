...
Some limits can be increased per request, as noted in the source article.

> [!NOTE]
> The source limits article describes preview limits that are subject to change. Always verify current limits before production launch.

## Size limits

The source article lists the following size limits.
...
The source limit for files and folders with additive permissions per container is 5k.
...
Use bounded retries and surface persistent failures to operations telemetry.
...
> [!NOTE]
> The source article states that API resource unit costs can change.

## API rate limits

The source article lists these API rate limits.
