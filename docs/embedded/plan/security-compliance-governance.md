...
For the source compliance details, see [Security and compliance](../compliance/security-and-compliance.md).
...
Compliance and SharePoint administrators enforce supported policies on containers and content by using Microsoft Purview, SharePoint admin capabilities, and PowerShell.
...
For an enterprise LOB app, the enterprise tenant usually owns and consumes the app.
...
The source article lists this PowerShell discovery flow:
...
The source compliance article describes support for:
...
Global Administrators and SharePoint Administrators can set and remove sensitivity labels on a SharePoint Embedded container by using SharePoint PowerShell.
