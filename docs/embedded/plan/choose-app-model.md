...
- The app registration is usually in the enterprise tenant.
...
The ISV app provides the user experience and calls Microsoft Graph. Customer content stays with the customer.
...
For an LOB app, the enterprise usually owns both.

For an ISV app, the ISV usually owns both.
...
For the registration flow, see [Register file storage container type application permissions](../getting-started/register-api-documentation.md).
...
The admin in the developer tenant establishes a valid billing profile when creating a standard container type.
...
| Configure standard billing | Enterprise/developer admin | ISV/developer admin |
...
1. Create the container type by using the guidance in [SharePoint Embedded container types](../getting-started/containertypes.md).
1. Configure billing by using [PAYG billing for SharePoint Embedded](../administration/billing/billing.md).
...
1. Register the container type in each consuming tenant by using [Register file storage container type application permissions](../getting-started/register-api-documentation.md).
